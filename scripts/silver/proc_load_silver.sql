...
BEGIN
    ...
    BEGIN TRY
		...
		REPLACE(cid, '-', '') cid,
		CASE 
			-- 1. Gestione dei valori NULL o stringhe vuote (dopo la pulizia)
			WHEN cntry IS NULL OR TRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), '')) = '' THEN 'n/a'

			-- 2. Confronto per la Germania (DE)
			WHEN UPPER(TRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), ''))) = 'DE' THEN 'Germany'

			-- 3. Confronto per gli Stati Uniti (US, USA)
			WHEN UPPER(TRIM(REPLACE(REPLACE(cntry, CHAR(13), ''), CHAR(10), ''))) IN ('US', 'USA') THEN 'United States'

			-- 4. Per tutti gli altri valori, restituisce il valore pulito
		...
		subcat,
		CASE 
				-- Cerca 'YES' dopo aver pulito e convertito in maiuscolo
				WHEN UPPER(TRIM(REPLACE(REPLACE(maintenance, CHAR(13), ''), CHAR(10), ''))) = 'YES' THEN 'Yes'
				
				-- Cerca 'NO' dopo aver pulito e convertito in maiuscolo
				WHEN UPPER(TRIM(REPLACE(REPLACE(maintenance, CHAR(13), ''), CHAR(10), ''))) = 'NO' THEN 'No'
				
				-- Opzionale: gestisce tutti gli altri casi (es. NULL, stringhe vuote, valori inattesi)
				ELSE NULL -- o 'n/a' a seconda di cosa preferisci
			END AS maintenance
